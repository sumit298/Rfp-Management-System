...
import { rfpApi, type ComparisonResult } from '@/lib/api';
...
export default function CompareProposals() {
    ...
    const fetchComparison = async () => {
        try {
            setLoading(true);
            const data = await rfpApi.getComparison(id!);
            setComparison(data);
        } catch (err: any) {
    ...
    const { proposals, recommendation } = comparison;
            ...
            <main className="container py-8">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <h2 className="text-xl font-bold mb-2">
                                {recommendation.vendorName || 'Best Proposal'}
                            </h2>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
                    {proposals.map((proposal) => {
                        const isRecommended = proposal.vendorId?._id === recommendation?.vendorId;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        ...
                                        <h3 className="font-semibold text-lg">
                                            {proposal.vendorId?.name || `Proposal #${proposals.indexOf(proposal) + 1}`}
                                        </h3>
                                ...
                                <div className="p-4 rounded-lg bg-muted/50 mb-4">
                                    ...
                                    <p className="text-2xl font-bold">{formatCurrency(proposal.pricing.totalCost)}</p>
                                </div>

                                {/* Details Grid */}
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    {proposal.terms.deliveryDays && (
                                        <div>
                                            <p className="text-xs text-muted-foreground">Delivery</p>
                                            <p className="text-sm font-medium">{proposal.terms.deliveryDays} days</p>
                                        </div>
                                    )}
                                    {proposal.terms.paymentTerms && (
                                        <div>
                                            <p className="text-xs text-muted-foreground">Payment</p>
                                            <p className="text-sm font-medium">{proposal.terms.paymentTerms}</p>
                                        </div>
                                    )}
                                    {proposal.terms.warranty && (
                                        <div className="col-span-2">
                                            <p className="text-xs text-muted-foreground">Warranty</p>
                                            <p className="text-sm font-medium">{proposal.terms.warranty}</p>
                                        </div>
                                    )}
                                </div>
                                ...
                                {proposal.pricing.items && proposal.pricing.items.length > 0 && (
                                    <div>
                                        <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                                setExpandedProposal(isExpanded ? null : proposal._id)
                                            }
                                            className="w-full justify-between"
                                        >
                                            <span>Item Breakdown ({proposal.pricing.items.length})</span>
                                            {isExpanded ? (
                                    ...
                                    {proposals.map((p) => (
                                        <th
                                            ...
                                            className={cn(
                                                ...
                                                p.vendorId?._id === recommendation?.vendorId &&
                                                'bg-muted/50'
                                            )}
                                        >
                                            {p.vendorId?.name || `Proposal #${proposals.indexOf(p) + 1}`}
                                            {p.vendorId?._id === recommendation?.vendorId && (
                            ...
                            <tbody>
                                ...
                                <tr className="border-b border-border">
                                    ...
                                    {proposals.map((p) => (
                                        ...
                                        >
                                            {formatCurrency(p.pricing.totalCost)}
                                        </td>
                                    ))}
                                </tr>
                                <tr className="border-b border-border">
                                    ...
                                    {proposals.map((p) => (
                                        ...
                                        >
                                           {p.terms.deliveryDays ? `${p.terms.deliveryDays} days` : '-'}
